...
class Grants
{
     ...
     * @param string|Table $tableName The Table or name of the table.
     * @return type
     */
    public static function current_user_can($grant, $tableName)
    {
        ...
        $sql = "SELECT * "
                . " FROM grants "
                . " WHERE (`table_name`='*' OR `table_name` = :table_name) AND `action` LIKE :grant ";
        $params = ['table_name' => $tableName, 'grant' => $grant];
        $grants = $db->query($sql, $params);
        var_dump($grants);
        return true; // current_user_can( $capability, $table_name );
    }
}
